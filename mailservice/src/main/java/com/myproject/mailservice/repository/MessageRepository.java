...
public interface MessageRepository extends JpaRepository<Message, Long>{
	...
	@Query(value="SELECT m.* FROM message m LEFT JOIN accounts a ON m.account_id = a.account_id WHERE a.user_id =? ORDER BY m.subject_mess DESC", nativeQuery = true)
	List<Message> findAllSortBySubject(Long id);
	
	@Query(value="SELECT m.* FROM message m LEFT JOIN accounts a ON m.account_id = a.account_id WHERE a.user_id =? ORDER BY m.from_message DESC", nativeQuery = true)
	List<Message> findAllSortBySender(Long id);
	
	@Query(value="SELECT m.* FROM message m LEFT JOIN accounts a ON m.account_id = a.account_id WHERE a.user_id =? ORDER BY m.date_time DESC", nativeQuery = true)
